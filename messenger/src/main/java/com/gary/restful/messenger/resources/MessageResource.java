...
import com.gary.restful.messenger.service.MessageService;
...
//	@Produces(MediaType.APPLICATION_JSON)
	...
	//当使用了@QueryParam注解的时候，在url中出现的参数如果有名为year的，那么它的值就会被传到int year中去
	// e.g. http://localhost:8080/messenger/webapi/messages?year=2018   会传入2018
	// 在URL中没有传入地址的话，默认就是0
	public List<Message> getMessage(@QueryParam("year") int year,
		...
		// if size>=0 then getAllMessages() will never be called
		if (start>=0 && size>0) {
